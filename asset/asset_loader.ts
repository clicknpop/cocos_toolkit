...
export class AssetLoader {
    ...
    private async load<T extends Asset>(type: typeof Asset, opt: AssetLoadOpt): Promise<T> {
        return new Promise(resolve => {
            ...
            if (!loader) {
                console.warn(`load asset failed, loader is null.`, type.name, opt.assetPath, opt.bundleName);
                return null;
            }

            loader.load(opt.assetPath, type, (e, asset) => {
                if (e) {
                    console.error(`load asset failed.`, type.name, opt.assetPath, opt.bundleName, e.message);
                    return null;
                }
